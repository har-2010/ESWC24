...
#  pas été utilisés !
from tools.pdfToCleanText import pdfToCleanText # pas utilisé dans la suite
...
class Corpus() :
    def __init__(
        ...
        self.ontology                               = pd.read_csv(source)[columns].dropna() # on a mis ici le fichier CSV, en enlevant 
...
#-----------------------------extract_from_pdf ne sert à rien
    ...
    # def extract terms ci-dessous ne sert à rien ----------------------------------------------
   ...
   #------------------------------------ def extract_terms_new(self) -> Corpus 
    ...
    def extract_terms_from_onto(self) :
        ...
        Returns:
            None
        """
       # self.extract_from_pdf(pdf_Path) # cette ligne ne sert à rien il faut que ja la vérifie
        
        terms = pd.unique(self.ontology[self.colums[0]]) # on met tous les termes à clusterer dans terms
        for term in terms :
    ...
    def _get_embedding(self, term) :
        ...
        return self.model.encode(term) # encode ca vient avec sentencestransofmers 
    ...
    def _parse_ontology(self, ontology, hypernym : Term | None = None):
        ...
        for cc, sub_concepts in ontology.items() :  #on associe les termes de chaque CC et on rajoute les termes à vocabylary et en plus on rajoute 
            term = Term(cc)
            self.vocabulary.append(term)
